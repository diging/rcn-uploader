...
@Service
public class LocalDescriptionsParserRegistry implements ILocalDescriptionsTagParserRegistry {

    @Autowired
    private ApplicationContext ctx;

    private Map<String, LocalDescriptionsTagParser> tagParsers;
    
    @PostConstruct
    public void init() {
        tagParsers = new HashMap<>();
        ctx.getBeansOfType(LocalDescriptionsTagParser.class).values()
     ...
     */
    @Override
    public void parseRecordTag(Node node, LocalDescriptions desc) {
        LocalDescriptionsTagParser handler = tagParsers.get(node.getNodeName());
        if (handler != null) {
