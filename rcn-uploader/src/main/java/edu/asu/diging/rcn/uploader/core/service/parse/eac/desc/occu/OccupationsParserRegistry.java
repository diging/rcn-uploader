...
@Service
public class OccupationsParserRegistry implements IOccupationsTagParserRegistry {

    @Autowired
    private ApplicationContext ctx;

    private Map<String, OccupationsTagParser> tagParsers;
    
    @PostConstruct
    public void init() {
        tagParsers = new HashMap<>();
        ctx.getBeansOfType(OccupationsTagParser.class).values()
     ...
     */
    @Override
    public void parseRecordTag(Node node, Occupations occupations) {
        OccupationsTagParser handler = tagParsers.get(node.getNodeName());
        if (handler != null) {
            handler.parse(node, occupations);
        }
    }
