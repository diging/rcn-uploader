...
@Service
public class OutlineParserRegistry implements IOutlineTagParserRegistry {

    @Autowired
    private ApplicationContext ctx;

    private Map<String, OutlineTagParser> tagParsers;
    
    @PostConstruct
    public void init() {
        tagParsers = new HashMap<>();
        ctx.getBeansOfType(OutlineTagParser.class).values()
     ...
     */
    @Override
    public void parseRecordTag(Node node, Outline outline) {
        OutlineTagParser handler = tagParsers.get(node.getNodeName());
        if (handler != null) {
            handler.parse(node, outline);
        }
    }
