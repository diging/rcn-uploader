...
@Service
public class DescriptionParserRegistry implements IDescriptionTagParserRegistry {

    @Autowired
    private ApplicationContext ctx;

    private Map<String, DescriptionTagParser> tagParsers;
    
    @PostConstruct
    public void init() {
        tagParsers = new HashMap<>();
        ctx.getBeansOfType(DescriptionTagParser.class).values()
     ...
     */
    @Override
    public void parseRecordTag(Node node, Description desc) {
        DescriptionTagParser handler = tagParsers.get(node.getNodeName());
        if (handler != null) {
